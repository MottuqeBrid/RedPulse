...
const ChangePasswordModal = ({ onClose }) => {
  ...
  const onSubmit = async (data) => {
    setSaving(true);
    try {
      const res = await axiosInstance.patch("user/update?fields=password", {
...
const ChangeEmailModal = ({ user, setUser, onClose }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data) => {
    setSaving(true);
    try {
      const res = await axiosInstance.patch("user/update?fields=email", {
        email: data.email,
      });
      if (res.data?.success) {
        setUser(res.data.data);
...
const ChangePhoneModal = ({ user, setUser, onClose }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (data) => {
    setSaving(true);
    try {
      const res = await axiosInstance.patch("user/update?fields=phone", {
        phone: data.phone,
      });
      if (res.data?.success) {
        setUser(res.data.data);
  ...
  return (
    <dialog className="modal modal-open">
      <div className="modal-box">
        <h3 className="font-bold text-lg mb-4">Change Phone Number</h3>
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <div className="form-control">
            <label className="label">
